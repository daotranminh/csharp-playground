...
namespace HelloWebApi.Models
{
    ...
    {
        private ISet<string> supportedCultures =
            new HashSet<string>() { "en-us", "en", "fr-fr", "fr" };
                                                                     ...
                                                                     CancellationToken cancellationToken)
        {
            ...
            {
                var headerValue = list.
                                    OrderByDescending(e => e.Quality ?? 1.0D).
                                    Where(e => !e.Quality.HasValue || e.Quality > 0.0D).
                ...
                // Case 2: Client is ok to accept anyhing we support except
                // the one explicitly specified as not prefered by setting q=0
                if (list.Any(e => e.Value == "*" && (!e.Quality.HasValue || e.Quality.Value > 0.0D)))
                {
                    var culture = supportedCultures.Where(sc => !list.
                                                                    Any(e => e.Value.Equals(sc, StringComparison.OrdinalIgnoreCase) &&
                                                                             e.Quality.HasValue &&
